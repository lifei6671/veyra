...
- 2026-09-05：确认订阅卡片“使用”立即切换到所选订阅的运行配置；保留其它已导入订阅，
  不将“使用”实现为仅影响下次启动的多订阅启停。具体变更见 DCR-018。
- 2026-09-05：明确 Veyra 的核心交付是参考 Clash Verge Rev 的 sing-box 桌面 UI；
  验收聚焦应用自有逻辑和最小内核集成，取消重复认证 sing-box 协议实现的任务前置条件。
...
项目核心目标是为 sing-box 提供易用、稳定的桌面 UI，页面组织、布局和交互参考 Clash Verge Rev。
领域模型、订阅归一化、配置转换、状态存储和运行管理服务于这些用户功能；不重复实现或认证内核协议能力。
...
### 1.2 产品与验证职责边界（2026-09-05 用户确认）

UI 参考仓库为 `E:/wx_lifeilin/github.com/clash-verge-rev`。后续功能以其首页/侧栏、
代理、订阅、规则、连接、日志、设置和托盘的页面组织与交互为依据，映射到 Veyra 的
订阅、节点、出口组和分流模型。参考项目的 Mihomo 配置/API、脚本及其专有功能不随 UI
参考自动进入 Veyra；既有 sing-box 单内核、平台版本和功能范围保持。

视觉风格必须与 Clash Verge Rev 保持一致，覆盖浅色/深色主题、背景层级、文字、边框、
选中/悬停/错误状态和图表配色。颜色直接对应参考项目的主题定义和组件实现，不自造色系。
当前首页使用系统浅色/深色偏好选择对应主题；后续主题设置沿用同一套配色。验收须记录
主题源码映射，并分别验证两种主题的实际渲染、切换和可读性。

字体沿用参考前端的加载方案：内置 `Twemoji.Mozilla.ttf` 并通过 `@font-face` 加载，
用于 Emoji/旗帜，普通文字沿用参考主题的系统字体栈，按钮和表单控件继承。字体文件随应用
构建打包，保留来源和许可说明，验证实际字体加载与渲染，不以字体名声明代替加载证据。

Veyra 负责界面输入/反馈、订阅与持久化、配置转换/check、运行生命周期、观测映射和
平台适配。sing-box 负责协议、数据转发、地址族、TLS/DNS 与内核探测算法。
测试须对应一项 Veyra 自有行为；真实内核仅做必要的配置/API/进程与代表性连通集成。
不建立 WireGuard/TCP/UDP/IPv6/DNS/TLS 全矩阵，不新建网络栈、校验和或 ACK 验证设施。
针对可复现产品缺陷可做有界诊断，但不将上游能力认证设为 UI 功能的完成条件。

保留 Veyra 自有安全策略的配置正反断言和适量已有集成证据，包括生成拒绝规则、
私有配置、鉴权、脱敏和 child 所有权；用户可见操作、失败提示和桌面行为必须验证。
本边界同时适用于当前和后续 Task；具体影响与历史证据处理见
`docs/decisions/DCR-017-ui-integration-boundary.md`，测试分层见 §143.1。

订阅标准化、统一节点模型、配置编译方式参考 Satelite Proxy 的架构思想。

### 1.3 订阅“使用”交互（2026-09-05 用户确认）

订阅卡片“使用”与 Clash Verge Rev 对齐：用户点击后立即生成、检查并应用选中订阅的运行配置，
当前运行内容切换到该订阅。支持管理多份订阅，但这次显式选择不把其它订阅节点自动合入执行配置；
其它订阅及其节点数据继续保留，不通过删除数据实现切换。本节优先于此前将卡片限定为只读、
仅启用/停用多订阅成员或仅影响下次显式启动的设计表述。

普通导入、编辑和更新仍是订阅管理操作，不因此自动切换运行配置。“使用”是独立的显式应用动作。
配置生成或 check 失败保留原实例；候选启动失败沿用 DCR-004：清理失败候选、呈现实际停止或
恢复待处理状态，不自动启动旧配置。只有候选 Ready 才能显示“正在使用”，不能以按钮点击、保存
或编译成功代替实际运行结果。验收覆盖成功切换、失败阶段、重复点击、重启与运行标识一致性，
不重复验证 sing-box 的内部协议能力。

变更记录为 `docs/decisions/DCR-018-subscription-use.md`。用户已确认上述产品语义；新增具体权限、
第三方依赖及尚未完成的技术设计审阅独立记录，不由该确认推定通过。
...
Veyra 验证生成配置中的拒绝规则及其优先级，并使用已有代表性集成证据检查接线；不重复认证
内核 ICMP 或协议/地址族行为。节点服务器域名使用系统 DNS；URLTest 目的域名保留成员协议原生解析行为。
...
1. 应用单测/组件交互：验证 Veyra 的输入归一化、状态保存、配置映射、错误响应、Toast、
   忙碌态、观测实例隔离和流量展示。保留边界/失败断言，不测试 sing-box 内部算法。
2. 最小真实集成：最终配置通过固定核心 check，同一最终字节启动到 Ready，固定鉴权 API
   读回并映射到当前实例，Stop 清理自有资源；需要流量时复用一个已有受控阳性场景。
   应用生成的拒绝策略保留结构/顺序/不能被用户规则覆盖的正反断言及已有代表性接线证据。
3. UI/桌面验收：用户可见的状态、操作反馈、订阅更新/节点选择/分流编辑等实际交互，以及
   隐藏/托盘恢复由相应功能 Task 验收；截图和交互证据不能被后台或 Mock 结果替代。

协议字段的 Parser/Compiler fixture 覆盖仍保留，它验证 Veyra 的映射；真实握手、包解析、
校验和、TCP ACK/重传、IPv6 转发、DNS/Host/SNI 内核矩阵不作为当前或后续 Task 的必需条件。
旧验证记录保留原身份与 PASS/FAIL/NOT_RUN；移出责任范围不代表其测试通过或缺陷修复。
如历史失败可定位到 Veyra 自有配置、进程/资源管理或用户交互，仍须在对应功能内处理。
版本/平台/权限/生产等已有边界不因测试范围调整而开放。
