...
tcp     {v, event:"tcp", run_id, requests:1, response_status:204,
         rx_tcp_packets:u64, tx_tcp_packets:u64, authenticated:true}
...
TCP 事件必须在请求校验和完整响应写成功之后发出；rx/tx_tcp_packets 都 >0，
来自真实内存 tun 边界，以“WG 已解密/待加密”为方向，排除握手和自测计数。
父断言 DUT Ready、唯一 URLTest 的 peer 请求/204 证据以及对应双向包计数，不把
单独 Ready、Clash API 在线或任意延迟数字当成 WG TCP 成功。
...
单测；`go test -timeout 60s ./...`、`go vet ./...`、gofmt 检查、模块校验和显式 build。
